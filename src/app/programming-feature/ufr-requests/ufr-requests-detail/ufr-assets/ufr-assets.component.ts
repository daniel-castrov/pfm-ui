...
@Component({
  ...
  templateUrl: './ufr-assets.component.html',
  styleUrls: ['./ufr-assets.component.scss']
})
export class UfrAssetsComponent implements OnInit {
      ...
      .then(
        resp => {
          ...
          fundingLine.forEach(option => {
            const asset = this.ufr.assets && this.ufr.assets.find(a => a.fundingLineId === option.id);
            if (!asset) {
              this.assetService.obtainAssetByFundingLineId(option.id).subscribe(
                assetRespose => {
                  this.ufr.assets.push(assetRespose.result);
                },
                error => {
                  this.dialogService.displayDebug(error);
                }
  ...
  private addEmptyRow() {
    ...
    const asset = this.ufr.assets.find(a => a.id === this.selectedAsset.id);
    ...
    saveOrUpdate(assetSummary, this.pomYear)
      ...
      .subscribe(
        assetResponse => {
          ...
          this.ufr.assets.find(a => a.id === this.selectedAsset.id).assetSummaries.splice(rowIndex, 1, assetResponse);
        },
        async error => {
          if (error.status === 400) {
  ...
  private performDelete(rowIndex: number) {
    ...
    this.resetState();
    this.ufr.assets.find(a => a.id === this.selectedAsset.id).assetSummaries.splice(rowIndex, 1);
  }
